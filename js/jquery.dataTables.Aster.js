...
(/** @lends <global> */function( $ ) {
	...
	function _fnAdjustColumnSizing ( settings )
	{
		...
		if ( settings.oFeatures.bAutoWidth !== false )
		{
            ...
            /**
             * Aster filter-enabled columns fix -
             * need to set max width to be able to correctly display overflow in table cells
             */
            columns[i].nTh.style.maxWidth = columns[i].sWidth;
			}
		}
	...
	function _fnAddData ( oSettings, aDataIn, nTr, anTds )
	{
		...
		for ( var i=0, iLen=columns.length ; i<iLen ; i++ )
		{
         ...
         //Aster fix - do not clear sType here
	...
	function _fnScrollDraw ( settings )
	{
      ...
      //Aster hack to exclude filter row
      if (headerCopy.find('tr').length>1){
         headerCopy.find('tr:eq(1)').remove();
      }
...
//      var nThs = _fnGetUniqueThs( settings, headerCopy );
//      var iLen, iVis, i;
//      for ( i=0, iLen=nThs.length ; i<iLen ; i++ )
//      {
//         iVis = _fnVisibleToColumnIndex( settings, i );
//         console.log('idx', i, iVis);
//         nThs[i].style.width = settings.aoColumns[iVis].sWidth;
//      }
      ...
      if ( footer ) {
		...
		_fnApplyToChildren( function(nToSize, i) {
          ...
          * need to set max width to be able to correctly display overflow in table cells
          */
         nToSize.style.maxWidth = headerWidths[i];
		}, headerTrgEls );
	...
	function _fnCalculateColumnWidths ( oSettings )
	{
		...
		else
		{
			...
			for ( i=0 ; i<visibleColumns.length ; i++ ) {
            /**
             * Aster fix for hidden columns issue - need to use a visible column index for header retrieval as
             * visibleColumns.length could be less than headerCells.length
             */
            var index=visibleColumns[i];
				column = columns[ index ];
	
				headerCells[index].style.width = column.sWidthOrig !== null && column.sWidthOrig !== '' ?
					_fnStringToCss( column.sWidthOrig ) :
					'';
            /**
             * Aster fix - remember to also set the maxWidth - will be needed for trimming text by CSS ellipsis
             */
            headerCells[index].style.maxWidth=headerCells[index].style.width;
			}
			...
			if ( scrollX )
			{
                ...
                * visibleColumns.length could be less than headerCells.length
                */
					outerWidth = $(headerCells[ visibleColumns[i] ]).outerWidth();
